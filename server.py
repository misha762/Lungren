#!/usr/bin/env python3
"""
Простий HTTP сервер для тестування PWA локально та на Render
"""
...
def main():
    ...
    # Визначаємо хост для різних середовищ
    HOST = "0.0.0.0" if os.environ.get('PORT') else ""  # 0.0.0.0 для Render, "" для локального
    
    with socketserver.TCPServer((HOST, PORT), CustomHTTPRequestHandler) as httpd:
        if os.environ.get('PORT'):
            # Режим Render
            print(f"🚀 Сервер запущено на Render!")
            print(f"🌐 URL: https://your-app-name.onrender.com")
            print(f"📱 Для iPhone: https://your-app-name.onrender.com")
        else:
            # Локальний режим
